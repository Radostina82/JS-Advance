function solve() {
  ...
  function buy(e){
   const futniture = Array.from (document.querySelectorAll('input[type="checkbox"]:checked')).
   ...
   for(let item of futniture){
     total+= item.price;
     decFactors += item.decFactor;
     names.push(item.name);
   }
   const result = `Bought furniture: ${names.join(', ')}.
Total price: ${total.toFixed(2)}
Average decoration factor: ${decFactors / futniture.length}`;
